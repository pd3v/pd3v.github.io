...
f = _ => {
  ...
  // Every 15 notes generate new phrasings
  if (counter()%15 == 0) {
  ...
  return {
    ...
    oct:Math.trunc(scaleTo(0,5,3,5,round(phrasing.reduce((accum, value)=>{
      accum += value;
      return accum;
...
Sequencer.generator(CMinorScale, f).synth('Synth',{a:rand(0,0.1),d:0,r:0.05}).start(ac,round(rand(90,130)));
